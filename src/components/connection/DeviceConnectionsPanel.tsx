...
function getLocalPort(conn: Connection, name: string) {
  return conn.srcDevice.toLowerCase() === name.toLowerCase() ? conn.srcPort : conn.dstPort;
}
function getRemotePort(conn: Connection, name: string) {
  return conn.srcDevice.toLowerCase() === name.toLowerCase() ? conn.dstPort : conn.srcPort;
}
function getLocalIp(conn: Connection, name: string) {
  return conn.srcDevice.toLowerCase() === name.toLowerCase() ? (conn.srcIp ?? "") : (conn.dstIp ?? "");
}
function getRemoteIp(conn: Connection, name: string) {
  return conn.srcDevice.toLowerCase() === name.toLowerCase() ? (conn.dstIp ?? "") : (conn.srcIp ?? "");
}
function getRemote(conn: Connection, name: string) {
  return conn.srcDevice.toLowerCase() === name.toLowerCase() ? conn.dstDevice : conn.srcDevice;
}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
        {deviceConns.length === 0 ? (
          <p className="text-[12px] italic text-faint">No connections.</p>
        ) : (
                  ...
                  <div className="space-y-1.5">
                    {[...groups.entries()].map(([remoteKey, groupConns]) => {
                      ...
                      return (
                        <div
                          key={remoteKey}
                          className="rounded-lg border border-line/40 bg-surface/30 px-2 py-1.5 transition-colors hover:bg-brand/8"
                          onContextMenu={(e) => {
                            e.preventDefault();
